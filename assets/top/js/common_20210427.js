$(function () {
  ...
  function rankingApiCall() {
    $.when(
      $.getJSON(
        getAPIHost() +
        'api/web_content/ranking/monthly?access_key=xr5hSVwnxmCehZ74&limit=10'
      ),
      $.getJSON(
        getAPIHost() +
        'api/web_content/ranking/weekly?access_key=xr5hSVwnxmCehZ74&limit=10'
      ),
      $.getJSON(
        getAPIHost() +
        'api/web_content/ranking/hourly?access_key=xr5hSVwnxmCehZ74&limit=10'
      )
    )
  ...
  function init(_ua) {
    ...
    songsTab.on('click', function (_ua) {
      ...
      // ソングタブクリックで#songsのheight変更(out of use 2021/4/27)
