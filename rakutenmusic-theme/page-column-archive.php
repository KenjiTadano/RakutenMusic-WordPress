<?php
/**
 * Template Name: 読む音楽ガイド アーカイブ
 *
 * コラムアーカイブページ用テンプレート。
 * このテンプレートを割り当てた固定ページの「子ページ」が記事一覧になります。
 * 各子ページ：サムネイル＝アイキャッチ画像、タイトル＝ページタイトル、日付＝更新日、
 * バッジ＝カスタムフィールド「rakutenmusic_column_badge」（feature / column / interview / ranking）で設定。
 *
...
$archive_items   = rakutenmusic_get_column_archive_items( $archive_page_id );
...
$record_url    = $assets_uri . '/column/img/common/img_record.png';
?>

<div id="page">
	<main class="l-content">
		<!-- メイン看板 -->
		<section class="hero-container" style="background-image: url(<?php echo esc_url( $bg_url ); ?>); background-color: lightgray; background-position: 50% 50%; background-size: cover; background-repeat: no-repeat;">
			<div class="articles-logo-container">
				<div class="articles-logo-set">
					...
					<h1 class="articles-title"><?php esc_html_e( '読む音楽ガイド', 'rakutenmusic-theme' ); ?></h1>
				</div>
				<div class="articles-line"></div>
				<h4 class="articles-subtitle"><?php esc_html_e( '新しい発見がここに', 'rakutenmusic-theme' ); ?></h4>
			</div>
			<div class="hero-desc"><?php esc_html_e( '楽天ミュージックが贈る、聴いても読んでも楽しめる音楽記事をお届け', 'rakutenmusic-theme' ); ?></div>
			<img src="<?php echo esc_url( $record_url ); ?>" alt="" class="top-right-image" />
			<img src="<?php echo esc_url( $record_url ); ?>" alt="" class="bottom-left-image" />
		</section>
	</main>
</div>
						...
						<div class="article-card">
							<div class="cover-img">
								<a href="<?php echo esc_url( $item['link'] ); ?>">
									...
									<?php else : ?>
										<span class="cover-img-placeholder"><?php esc_html_e( '画像なし', 'rakutenmusic-theme' ); ?></span>
									<?php endif; ?>
								</a>
							</div>
							<div class="tags">
								<div class="<?php echo esc_attr( $item['badge_class'] ); ?>"><?php echo esc_html( $item['badge_label'] ); ?></div>
							</div>
							<div class="name">
								<p><?php echo esc_html( $item['title'] !== '' ? $item['title'] : __( '（タイトルなし）', 'rakutenmusic-theme' ) ); ?></p>
							</div>
							<div class="date">
								<p><?php echo esc_html( $item['date'] !== '' ? $item['date'] : '—' ); ?></p>
							</div>
						</div>
					</li>
					<?php endforeach; ?>
